...
int findIPAddress(char myIps[4][4], char ip[4]);
...
int main(){
  ...
  scanFromFile("r2-table.txt", table1); 
	            ...
	            if(ipRow!=-1){
                  ...
                  if(timeout==2){
                    ...
                    ++timeout;
		          	...
		          	// ip address wasn't in the table, so send ICMP destination unreachable error
...
int createICMPError(char *buff, int type, int code){
	...
	// set the destination ip address in the ip header
...
int tableLookup(char ipTable[5][9], char destIP[4]){
...
int createTable(char *table, char ipTable[5][9]){
...
// it in ip
int getIP(char ipTable[5][9], char ip[4], int row){
...
// returns negative one if it is not found
int findMacAddress(char myMacs[4][6], char mac[6]){
  int i;
  int j;
  int count = 0;
  for(i=0; i<4; ++i){
